prmLstFunc<-function(fileLoc){
  ...
  for(i in c(1:nrow(prmFile))){
    prmTmp<-paramsFunc(
      ...
      gamma_1_Val = ifelse(prmFile$GammaVer[i]==1,runif(1,-0.6,3.6),0),
      gamma_2_Val = ifelse(prmFile$GammaVer[i]==2,runif(1,-0.6,3.6),0),
      ...
      epsilon_Val = ifelse(prmFile$Epsilon[i]>=1,runif(1,-0.6,3.6), 0),
      rho_Val = ifelse(prmFile$Rho[i]>=1,runif(1,-0.6,3.6), 0),
      S2_val=ifelse(prmFile$EnvOsc[i]==T,runif(1,1,100),0),
      zeta_p=ifelse(prmFile$zeta[i]==T,runif(1,0.1,1),0),
      pcrProb2=ifelse(prmFile$zeta[i]==T,runif(1,0.1,1),0),
      Phi2_val = ifelse(prmFile$EnvOsc[i]==T,runif(1,0.1,3.3),0),
