...
plotEnvForc <- function(resultsFile,burn,modNums,saveLoc,prmFile,thin=10) {
   ...
   params<- colMedians(gg)
    ...
    if (i == 3) {
      ...
      geom_line(colour = "purple", size = 1.5) +
        ...
        ylab(expression(Transition~rate:~L%->%I~(epsilon[t] ~ year ^ -1))) +
        geom_ribbon(
    ...
    if (i == 6) {
      ...
      G6 <- ggplot(sDriveGam, aes(y = sDriveGam, x = date)) +
        ...
        theme_bw(base_size = 20) +
        scale_x_date(date_breaks = "2 months" , date_labels = "%b") +
        ylab(expression(Transition~rate:~I%->%R~(gamma[t] ~ year ^ -1))) +
        geom_ribbon(
          aes(
    ...
    if (i == 8) {
      ...
      G8 <- ggplot(sDriveOmeg, aes(y = sDriveOmeg, x = date)) +
        ...
        theme_bw(base_size = 20) +
        scale_x_date(date_breaks = "2 months" , date_labels = "%b") +
        ylab(expression(Transition~rate:~R%->%S~(omega[t] ~ year ^ -1))) +
        geom_ribbon(
          aes(
  ...
  ggRt <-
    ggarrange(G4 + ggtitle("A) SILI"),
              G3 + ggtitle("B) SILI (No Maternal Immunity)"),
              G6 + ggtitle("C) SIR"),
              G8 + ggtitle("D) SIRS"),
              common.legend = T)
  print(ggRt)
