lpriorBoonah <- function(parms) with(parms, {
  gamma2_prior<-ifelse(parms$gamma_2_Val!=0,dunif(parms$gamma_2_Val,min=-0.69897 ,max=2.56,log=T),dunif(0.5,min=-0.69897,max=2.56,log=T))
  zeta_s_prior<-dunif(parms$zeta_s,min=0 ,max=1,log=T)
  omega2_prior<-ifelse(parms$omega_2_Val!=0,dunif(parms$omega_2,min=0,max=100,log=T),dunif(1,min=0,max=100,log=T))
  epsilon_prior<-ifelse(parms$epsilon_Val!=0, dunif(parms$epsilon,min=-0.69897,max=2.56,log=T),dunif(0.5,min=-0.69897,max=2.56,log=T))
  kappa_prior<-dnorm(parms$kappa,mean=3.60206,sd=0.1,log=T)
  R0_prior<-dunif(parms$R0_Val,min=1,max=100,log=T)
  rho_prior<-ifelse(parms$rho_Val!=0,dunif(parms$rho_Val,min=-0.69897,max=2.56,log=T),dunif(0.5,min=-0.69897,max=2.56,log=T))
  ...
  S2_val_prior<-ifelse(parms$c_val2==1, dunif(parms$S2_val,min=0.01,max=200,log=T),dunif(0.1,min=0.01,max=200,log=T))
